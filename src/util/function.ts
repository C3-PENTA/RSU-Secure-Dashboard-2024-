...
const formatTimestamp = (inputDate: string, inputTime: string) => {
  const combinedInput = `${inputDate} ${inputTime}`;
  const parsedDate = moment(combinedInput, 'MM-DD HH:mm:ss').toDate()
  return parsedDate;
}
